...
{
    const GameState &gs = pos.states[depth];
    ...
    {
        pos.makemove(moves[i], depth, Modern);

        if (pos.isRoyalSafe(gs.turn, depth + 1))
        {
            pos.undomove(moves[i], depth + 1);
            continue;
        }
